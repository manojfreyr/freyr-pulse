 ...
 * "Top services to lead with" is derived from the ranked Service Catalog matcher
 * output (recommendations) — not from any stored per-company list.
 */
export function WhyCareCard({
  ...
  const topServices = recommendations.slice(0, 5).map((m) => m.service);
