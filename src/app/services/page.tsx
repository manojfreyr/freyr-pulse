...
import { emptyService } from "@/lib/catalog/helpers";
...
export default function ServicesPage() {
  ...
  function handleSave(service: FreyrService) {
    setEditing(null);
    setError(null);
    setCatalog((list) => (list.some((s) => s.id === service.id) ? list.map((s) => (s.id === service.id ? service : s)) : [...list, service]));
    api
      .saveService(service)
      .catch((e: Error) => setError(`Could not save “${service.serviceName}”: ${e.message}`))
      .finally(refresh); // always reconcile with the server (no optimistic masking)
  }
  function handleDelete(id: string) {
    ...
    api
      .deleteService(id)
      .catch((e: Error) => setError(`Could not delete service: ${e.message}`))
      .finally(refresh);
  }
  function handleToggle(id: string) {
    const target = catalog.find((s) => s.id === id);
    if (!target) return;
    const next = { ...target, activeStatus: !target.activeStatus };
    setError(null);
    setCatalog((list) => list.map((s) => (s.id === id ? next : s)));
    api
      .saveService(next)
      .catch((e: Error) => setError(`Could not update service: ${e.message}`))
      .finally(refresh);
  }
  async function handleReset() {
    ...
    setError(null);
    try {
      setCatalog(await api.resetCatalog());
    } catch (e) {
      setError((e as Error).message);
    }
  }
  ...
  return (
    <div>
      <section className="spread" style={{ marginBottom: 22, flexWrap: "wrap", gap: 14 }}>
        <div>
          ...
          <p className="soft" style={{ margin: 0, maxWidth: 640 }}>
            ...
            Edits are saved to the shared team catalog (Supabase, with an in-memory fallback).
          </p>
        </div>
      ...
      {error && (
        <div className="card" style={{ marginBottom: 18, boxShadow: "none", borderColor: "#e6c3c3", background: "var(--danger-soft)" }}>
          <div className="small" style={{ color: "var(--danger)" }}>
            <span className="strong">Save failed.</span> {error}
            <div className="tiny muted" style={{ marginTop: 4 }}>Run <span className="mono">/api/debug</span> to see the database error. The list below reflects what&rsquo;s actually stored.</div>
          </div>
        </div>
      )}
