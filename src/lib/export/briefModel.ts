...
export function buildAccountBrief(
  ...
  const oi = buildOpportunityIntelligence(company, services);
  const primaryPersonaId = defaultPersona(matched, opts.personaId ?? oi.stakeholders[0]?.id);
  ...
  // ---- why now (from the opportunity engine — same ranking the dashboard shows) ----
  const whyNow: BriefTrigger[] = oi.whyNow.map((t) => ({
    headline: t.headline,
  ...
  // ---- overall strength (from the opportunity engine) ----
  const overallStrength = oi.evidenceStrength;
  ...
  return {
    ...
    opportunity: {
      ...
      reasons: oi.whyCare,
      nextBestAction: oi.nextActions[0]?.action ?? company.opportunity.nextBestAction,
      overallStrength,
    },
