...
/**
 * Machine-readable origin of a signal/citation. Drives the confidence trust map
 * (src/lib/signals/sourceTrust.ts). Distinct from SourceCitation.type, which is
 * the human-facing grouping label kept for backward compatibility.
 */
...
export interface SourceCitation {
  ...
  url: string;
  /** When the underlying data was retrieved. Phase 1: placeholder string. */
  ...
  // ---- Phase 2A.0b provenance (all optional; populated by live connectors) ----
  /** Stable id when persisted in the citations table. */
  id?: string;
  /** Machine-readable source type (drives the confidence trust map). */
  sourceType?: SignalSourceType;
  /** Publisher / issuer, e.g. "Reuters" or the company name. */
  publisher?: string;
  /** Event or publication date (ISO). */
  publishedAt?: string;
  /** Machine timestamp of retrieval (ISO) — distinct from the legacy dateRetrieved. */
  retrievedAt?: string;
  /** The grounding snippet that supports the claim. */
  snippet?: string;
}
...
/** A normalized company signal used for matching + display. (Defined here to
 *  avoid a cycle; re-exported from src/lib/signals/companySignals.ts.) */
...
/** Per-source outcome of one enrichment run (for the UI run log). */
...
/** Live-research output stored on a company (Phase 2A.0c). Folded into
 *  deriveSignals so the dashboard + matcher reflect live data. */
...
export interface FreyrService {
  ...
  /** Optional reference to a ServiceType master entity (V2.0-A refinement). Replaces the
   *  free-text serviceLine over time; unset on existing services, so behavior is unchanged. */
...
export interface Company {
  ...
  sources: SourceCitation[];
  /** Live-research output (Phase 2A.0c); absent until researched. */
