...
/** Delete by id and CONFIRM a row was removed. PostgREST returns success with 0
 *  rows when RLS filters the delete's row-set (RLS on / no DELETE policy, typical
 *  when using the anon key) — that silent no-op is surfaced here instead of being
...
async function deleteById(table: string, id: string, ctx: string): Promise<void> {
  ...
  if (!data || data.length === 0) {
    throw new Error(
      `${ctx}: DELETE removed 0 rows. The row was not deleted — either it does not exist, or DELETE is blocked by RLS / a missing DELETE grant (common when the app uses the anon key). Set SUPABASE_SERVICE_ROLE_KEY (bypasses RLS), or run supabase/fix-rls.sql (now disables RLS and grants DELETE).`,
    );
  }
}
