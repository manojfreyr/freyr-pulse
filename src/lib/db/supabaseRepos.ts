...
import type { AccountRepo, CacheEntry, CacheRepo, CatalogRepo, CompanyRepo, EnrichmentJob, JobRepo, Repos, ServiceTypeRepo, SegmentRepo, MarketRepo, SalesAssetRepo, ApplicabilityRepo, ServiceAssetRepo } from "./repositories";
import type { CustomerSegment, Market, SalesAsset, ServiceApplicabilityRule, ServiceAssetLink, ServiceType } from "@/lib/commercial/types";
import { SERVICE_TYPES_SEED, SEGMENTS_SEED, MARKETS_SEED } from "@/lib/commercial/seed";
...
/** Delete by id and CONFIRM via an INDEPENDENT re-read (the source of truth).
 *  We don't trust `.delete().select()`'s returned-row count: PostgREST can return
 *  zero rows from a successful delete depending on the representation header, which
 *  would look like a failure even though the row is gone. Re-reading avoids that
 *  false negative while still catching a genuine no-op (row still present). */
async function deleteById(table: string, id: string, ctx: string): Promise<void> {
  const del = await getSupabase().from(table).delete().eq("id", id);
  if (del.error) fail(ctx, del.error);
  const check = await getSupabase().from(table).select("id").eq("id", id).maybeSingle();
  if (check.error) fail(`${ctx}(verify)`, check.error);
  if (check.data) {
    throw new Error(
      `${ctx}: row still present after DELETE — the delete was filtered/blocked. With the service-role key this should not occur; verify SUPABASE_SERVICE_ROLE_KEY is the key in use.`,
    );
  }
}
...
const serviceTypeRepo: ServiceTypeRepo = {
  async list() {
    const { data, error } = await getSupabase().from(TABLES.serviceTypes).select("data").order("display_order", { ascending: true });
    if (error) fail("serviceTypes.list", error);
    const rows = (data ?? []).map((r) => r.data as ServiceType);
    if (rows.length > 0) return rows;
    const seed = SERVICE_TYPES_SEED.map((t) => ({ id: t.id, name: t.name, status: t.status, display_order: t.displayOrder, data: t, updated_at: nowIso() }));
    const ins = await getSupabase().from(TABLES.serviceTypes).upsert(seed, { onConflict: "id" });
    if (ins.error) fail("serviceTypes.list(seed)", ins.error);
    return [...SERVICE_TYPES_SEED];
  },
  async getById(id) {
    const { data, error } = await getSupabase().from(TABLES.serviceTypes).select("data").eq("id", id).maybeSingle();
    if (error) fail("serviceTypes.getById", error);
    return (data?.data as ServiceType) ?? SERVICE_TYPES_SEED.find((t) => t.id === id) ?? null;
  },
  async upsert(serviceType) {
    const stamped = { ...serviceType, updatedAt: nowIso() };
    const { error } = await getSupabase()
      .from(TABLES.serviceTypes)
      .upsert({ id: stamped.id, name: stamped.name, status: stamped.status, display_order: stamped.displayOrder, data: stamped, updated_at: nowIso() }, { onConflict: "id" });
    if (error) fail("serviceTypes.upsert", error);
    return stamped;
  },
  async remove(id) {
    await deleteById(TABLES.serviceTypes, id, "serviceTypes.remove");
  },
};
...
const segmentRepo: SegmentRepo = {
  ...
  async upsert(segment) {
    const stamped = { ...segment, updatedAt: nowIso() };
    const { error } = await getSupabase()
  ...
  async remove(id) {
    await deleteById(TABLES.segments, id, "segments.remove");
  },
};
...
const marketRepo: MarketRepo = {
  ...
  async upsert(market) {
    const stamped = { ...market, updatedAt: nowIso() };
    const { error } = await getSupabase()
      .from(TABLES.markets)
      .upsert({ id: stamped.id, name: stamped.name, region: stamped.region, regulatory_body: stamped.regulatoryBody, status: stamped.status, display_order: stamped.displayOrder, data: stamped, updated_at: nowIso() }, { onConflict: "id" });
    if (error) fail("markets.upsert", error);
    return stamped;
  },
  async remove(id) {
    await deleteById(TABLES.markets, id, "markets.remove");
  },
};
...
const salesAssetRepo: SalesAssetRepo = {
  ...
  async remove(id) {
    await deleteById(TABLES.salesAssets, id, "salesAssets.remove");
  },
};
...
const applicabilityRepo: ApplicabilityRepo = {
  ...
  async remove(id) {
    await deleteById(TABLES.applicability, id, "applicability.remove");
  },
};
...
const serviceAssetRepo: ServiceAssetRepo = {
  ...
  async remove(id) {
    await deleteById(TABLES.serviceAssets, id, "serviceAssets.remove");
  },
};
