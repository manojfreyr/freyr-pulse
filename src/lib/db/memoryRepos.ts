...
const g = globalThis as unknown as {
  ...
  __fpServiceTypes?: Map<string, ServiceType>;
...
// Commercial Enablement (V2.0-A): master entities seeded from code; the rest start empty.
const serviceTypes = (g.__fpServiceTypes ??= new Map(SERVICE_TYPES_SEED.map((t) => [t.id, { ...t }])));
...
const serviceTypeRepo: ServiceTypeRepo = {
  async list() {
    return Array.from(serviceTypes.values()).sort((a, b) => a.displayOrder - b.displayOrder);
  },
  async getById(id) {
    return serviceTypes.get(id) ?? null;
  },
  async upsert(serviceType) {
    const stamped = { ...serviceType, updatedAt: nowIso() };
    serviceTypes.set(stamped.id, stamped);
    return stamped;
  },
  async remove(id) {
    serviceTypes.delete(id);
  },
};
...
const segmentRepo: SegmentRepo = {
  ...
  async upsert(segment) {
    ...
    segments.set(stamped.id, stamped);
    return stamped;
  },
  async remove(id) {
    segments.delete(id);
...
const marketRepo: MarketRepo = {
  ...
  async upsert(market) {
    ...
    markets.set(stamped.id, stamped);
    return stamped;
  },
  async remove(id) {
    markets.delete(id);
