...
import type { CompanySignal } from "@/lib/signals/companySignals";
 ...
 * real sources without changing the rest of the app.
 *
 * Phase 2 flow (the seam these interfaces define):
 *   connector.fetchSignals(query)  →  CompanySignal[]   (per source, with provenance)
 *     → mergeSignals(structuredSignals, ...allConnectorSignals)
 *       → matcher / scoring / "Signals" UI
 *
 * Connectors emit normalized CompanySignal[] (the natural shape of FDA/SEC/news
 * facts) rather than whole Company objects. The legacy `fetchCompanyData`
 * (Partial<Company>) remains for sources that genuinely return profile fields.
 */
...
export interface DataSourceConnector {
  ...
  /**
   * Phase 2 (preferred): emit normalized signals with provenance for this
   * source. Optional so Phase 1 placeholders need not implement it.
   */
  fetchSignals?(query: { name?: string; website?: string }): Promise<CompanySignal[]>;
}
...
/**
 * Phase 2 entry point (stubbed). Will fan out to enabled connectors, collect
 * each source's `fetchSignals()` output, `mergeSignals(...)` them (with the
 * structured-profile signals from deriveSignals) into one provenance-aware
 * signal set, and hand that to the matcher/scoring + a "Signals" UI. For now it
 * signals that live research isn't available yet.
 */
