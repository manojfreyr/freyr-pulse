...
export function matchServices(company: Company, services: FreyrService[], limit = 8): MatchedService[] {
  ...
  const directPainServiceIds = new Set(company.painPoints.map((p) => p.relevantServiceId));
  const topIds = new Set(company.opportunity.topServiceIds);
  ...
  for (const service of services) {
    ...
    if (topIds.has(service.id)) score += 25;
    ...
    const confidence: ConfidenceRating =
      directPain || topIds.has(service.id) ? "Likely" : uniqueReasons.length + matchedTriggers.length >= 2 ? "Inferred" : "Inferred";
