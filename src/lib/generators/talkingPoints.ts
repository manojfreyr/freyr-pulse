...
export function generateTalkingPoints(
  ...
  // Phase 2B: prefer the strongest cited event behind a service this persona owns.
  const personaMatch = matched.find((m) => m.service.targetPersonas.includes(personaId)) ?? matched[0];
  const evEvent = personaMatch?.evidence?.find(
    (e) => e.category === "trigger" || e.category === "regulatory" || e.category === "pipeline",
  );
  const triggerPhrase = evEvent
    ? evEvent.signalLabel.toLowerCase()
    : topTrigger
      ? topTrigger.headline.toLowerCase()
      : "recent developments";
