import type { Company, ConfidenceRating, SignalSourceType, SourceCitation } from "@/lib/types";
...
export interface CompanySignal {
  ...
  /** Provenance: where this signal came from (human-readable). */
  source: string;
  /** Machine-readable source type (Phase 2A.0b) — drives the trust map. */
  sourceType?: SignalSourceType;
  /** Structured citations (Phase 2A.0b) — populated by live connectors. */
  citations?: SourceCitation[];
  /** When the signal was observed/published (ISO) — for recency (Phase 2A.0b). */
  observedAt?: string;
