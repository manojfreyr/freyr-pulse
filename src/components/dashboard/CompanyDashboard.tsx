...
import { loadCatalog } from "@/lib/store/serviceCatalog";
import { matchServices } from "@/lib/generators/serviceMatch";
import { isUserCompanySaved, saveUserCompany } from "@/lib/store/companyStore";
...
export function CompanyDashboard({ company }: { company: Company }) {
  ...
  // Live Service Catalog (localStorage). Recommendations are computed from it.
  const [catalog, setCatalog] = useState<FreyrService[]>(() => loadCatalog());
  ...
  const tabs = [
    {
      ...
      content: (
        <div className="stack" style={{ gap: 20 }}>
          <WhyCareCard company={company} serviceById={serviceById} />
...
function PlaceholderBanner({ company }: { company: Company }) {
  ...
  useEffect(() => { setSaved(isUserCompanySaved(company.id)); }, [company.id]);
      ...
      <div className="spread" style={{ gap: 16, flexWrap: "wrap" }}>
        <div style={{ minWidth: 0 }}>
          ...
          <p className="small soft" style={{ margin: 0, maxWidth: 640 }}>
            ...
            <span className="mono"> Unknown</span> until live research runs. In Phase 2, the data-source connectors
            will enrich and re-score this profile automatically.
          </p>
        </div>
        <div className="row-wrap" style={{ gap: 10 }}>
          {saved ? (
            ...
            <button className="btn btn-primary btn-sm" onClick={() => { saveUserCompany(company); setSaved(true); }}>
              Save company for future enrichment
            </button>
          )}
          <button className="btn btn-sm" disabled title="Live research is planned for Phase 2">
            Research again (Phase 2)
          </button>
        </div>
      </div>
    </div>
