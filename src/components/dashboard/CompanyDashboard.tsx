...
import { OpportunityIntelligencePanel } from "@/components/dashboard/OpportunityIntelligence";
import { ImproveProfile } from "@/components/dashboard/ImproveProfile";
import { buildOpportunityIntelligence } from "@/lib/opportunity/engine";
import { matchServices } from "@/lib/generators/serviceMatch";
import * as api from "@/lib/client/api";

export function CompanyDashboard({ company: initialCompany, autoResearch = false }: { company: Company; autoResearch?: boolean }) {
  ...
  const opportunity = useMemo(
    () => buildOpportunityIntelligence(company, catalog.filter((s) => s.activeStatus)),
    [company, catalog],
  );
  ...
  const tabs = [
    {
      ...
      content: (
        <div className="stack" style={{ gap: 20 }}>
          <WhyCareCard company={company} recommendations={recommendations} oi={opportunity} />
          <OpportunityIntelligencePanel oi={opportunity} />
  ...
  return (
    <div>
      ...
      <ResearchPanel company={company} onResearched={setCompany} autoStart={autoResearch} />
      <ImproveProfile company={company} onSaved={setCompany} />
