...
export function ResearchPanel({ company, onResearched, autoStart = false }: { company: Company; onResearched: (c: Company) => void; autoStart?: boolean }) {
  ...
  const websiteSkipped = !company.website || company.website === "Unknown";
  ...
  async function run() {
    ...
    try {
      ...
      while (guard++ < maxLoops) {
        ...
        if (job.company) latest = job.company; // authoritative, from the writing response
        if (job.status === "done" || job.status === "error") break;
        if (!job.steps.some((s) => s.status === "pending")) break;
      }
      // Prefer the company returned by the run (robust to read-after-write skew);
      // fall back to a fresh fetch only if needed.
      const fresh = latest ?? (await api.getCompany(company.id));
  ...
  useEffect(() => {
    if (autoStart && !autoFired.current && phase === "idle" && !company.enrichment?.lastResearchedAt) {
      ...
      // Strip ?onboard=1 so a later refresh never re-triggers auto-research.
      try {
    ...
    <div className="card fade-in" style={{ marginBottom: 20, boxShadow: "none", borderColor: "var(--accent-line)", background: "var(--accent-soft)" }}>
      ...
      {(running || phase === "done") && steps.length > 0 && (
        <div className="row-wrap" style={{ gap: 8, marginTop: 12 }}>
          {steps.map((s) => (
            <span key={s.source} className="chip" style={{ background: "var(--surface)" }} title={s.error || undefined}>
              {SOURCE_LABEL[s.source] ?? s.source}:{" "}
              <span className="mono" style={{ marginLeft: 4 }}>
                {s.status === "error" ? `failed${s.error ? ` — ${s.error}` : ""}` : s.status === "skipped" ? `skipped${s.error ? ` (${s.error.toLowerCase()})` : ""}` : s.status}
              </span>
            </span>
          ))}
        </div>
      ...
      {websiteSkipped && (
        <p className="small" style={{ marginTop: 12, marginBottom: 0, color: "var(--warn, #92400e)" }}>
          Website unavailable - please add a company website to improve research.
        </p>
      )}
